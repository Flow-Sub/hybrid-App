    ...
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 to-black aspect-square shadow-2xl">
        ...
        <div className="absolute inset-0 bg-black/95 backdrop-blur-sm z-20 flex flex-col items-center justify-center p-6">
          ...
          <div className="w-full space-y-3">
            {cameras.map((camera, index) => {
              const isRear = camera.label.toLowerCase().includes('back') || 
                           camera.label.toLowerCase().includes('rear') ||
                           (index === 0 && cameras.length > 1);
              const isFront = camera.label.toLowerCase().includes('front') || 
                            camera.label.toLowerCase().includes('user') ||
                            (index === 1 && cameras.length > 1);
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    <p className="text-white font-bold text-base">
                      {isRear ? 'Rear Camera' : isFront ? 'Front Camera' : camera.label}
                    </p>
                    <p className="text-gray-400 text-xs font-semibold mt-0.5">
                      {isRear ? 'Recommended for scanning' : 'Selfie camera'}
                    </p>
                  </div>
                </button>
              );
      ...
      {isScanning && (
        <>
          ...
          {/* Corner Frames */}
          <div className="absolute top-10 left-10 w-10 h-10 border-t-4 border-l-4 border-white pointer-events-none rounded-tl-lg"></div>
          <div className="absolute top-10 right-10 w-10 h-10 border-t-4 border-r-4 border-white pointer-events-none rounded-tr-lg"></div>
          <div className="absolute bottom-10 left-10 w-10 h-10 border-b-4 border-l-4 border-white pointer-events-none rounded-bl-lg"></div>
          <div className="absolute bottom-10 right-10 w-10 h-10 border-b-4 border-r-4 border-white pointer-events-none rounded-br-lg"></div>
